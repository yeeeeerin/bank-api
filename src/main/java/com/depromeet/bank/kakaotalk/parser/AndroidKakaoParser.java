...
public class AndroidKakaoParser implements KakaoParser {

    private static final String REGEX_DATE = "\\d{4}년 \\d{1,2}월 \\d{1,2}일 오[전|후] \\d{1,2}:\\d{1,2}";
    private static final Pattern PATTERN_DATE_GROUP = Pattern.compile("(\\d{4})년 (\\d{1,2})월 (\\d{1,2})일 (오[전|후]) (\\d{1,2}):(\\d{1,2})");
    private static final Pattern PATTERN_MESSAGE_FILE_PREFIX = Pattern.compile(".*님과 카카오톡 대화$");
    private static final Pattern PATTERN_MESSAGE_FILE_SAVED = Pattern.compile("저장한 날짜 : " + REGEX_DATE + "$");
    private static final Pattern PATTERN_MESSAGE_SYSTEM_INFORMATION = Pattern.compile(REGEX_DATE + ", [^:]*");
    private static final Pattern PATTERN_MESSAGE_SYSTEM_DATE = Pattern.compile(REGEX_DATE + "$");
    ...
    public void parse(String content) {
        ...
        chatMessages = messages.stream()
                .map(message -> ChatMessage.of(message, PATTERN_MESSAGE_USER, PATTERN_DATE_GROUP))
